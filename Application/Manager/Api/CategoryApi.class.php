...
class CategoryApi {
     ...
     * $only_allow 和$prohibit共同使用  $prohibit中的ID记录只显示$only_allow中的操作方法
     */
    function handleOperateG($list = array(), $model = '', $prohibit = array(), $only_allow = array('add_child','edit','set_status','delete')) {
        //空数组返回空
        if(empty($list)) { return array(); }
        //
        foreach($list as $key => $value) {
            ...
            //判断是否是二级分类
            $twoLv = '';
            if(!empty($value['hotBrand'])){
                $twoLv =  "<a href=\"#myModal\" role=\"button\"  data-toggle=\"modal\" data-catid='{$value['id']}' title='热门品牌' class='tip-bottom hotBrand' data-value='{$value['hot_brand']}'>
            ...
            if($prohibit && in_array($value['id'],$prohibit) && $only_allow) {
                ...
                $html = $add_child.$edit.$set_status.$delete.$twoLv;
            }
     ...
     * $only_allow 和$prohibit共同使用  $prohibit中的ID记录只显示$only_allow中的操作方法
     */
    function handleOperate2($list = array(), $model = '', $prohibit = array(), $only_allow = array('add_child','edit','set_status','delete')) {
        //空数组返回空
        if(empty($list)) { return array(); }
        //
        foreach($list as $key => $value) {
            //添加子分类操作的html
            $add_child = "<a href='".U(''.MODULE_NAME.'/'.$model.'/add/id/'.$value['id'].'')."' title='添加部门' class='tip-bottom'>
                            <span class='label label-warning'>+部门</span></a>&nbsp";
            //编辑分类的HTML
            $edit =  "<a href='".U(''.MODULE_NAME.'/'.$model.'/update/parent_id/'.$value['parent_id'].'/id/'.$value['id'].'')."' title='编辑' class='tip-bottom'>
                        <span class='label label-success'>编辑</span></a>&nbsp;";
            //删除操作的html
     ...
     * 获取树状下拉菜单
     */
    function getSelect2($list = array(), $field_name = '', $index_value = '', $index_field = 'id') {
        //判空
        if(empty($list)) return '';
        //将数据转换成树状结构
        $tree_data = list_to_tree($list);
        //下拉模板
        $template = "<li data-value='{id}' data-title='{top_class}{department_name}'><a href='javascript:void(0)'>{top_class}{department_name}</a></li>";
        //设置初始数据
        api('Tree/init',array($tree_data,$template,array('id','department_name')));
        //生成树状下拉菜单
        $html = api('Tree/toSelectFormatTree',array($index_value, $index_field));
        //html结构
        $select = "<div class='btn-group'>
