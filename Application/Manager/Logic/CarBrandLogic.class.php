 ...
 * 商品
 */
class CarBrandLogic extends BaseLogic {
...
//        }
        ...
        $result = D('CarBrand')->getList($param);
       ...
       //dump(D('Goods')->getLastSql()) ;
        ...
        foreach($result['list'] as $k=>$v){
            if(!empty($v['brand_logo'])){
                ...
                $result['list'][$k]['true_brand_logo'] = M('file')->field('path')->where("id={$v['true_brand_logo']}")->find()["path"];
            }
    ...
    function findRow($request = array()) {
        ...
        $row = D('CarBrand')->findRow($param);

        if(!$row) {
            $this->setLogicError('未查到此记录！'); return false;
        }
        $row['brand_logo'] = api('System/getFiles',array($row['brand_logo']));
        $row['true_brand_logo'] = api('System/getFiles',array($row['true_brand_logo']));
//        dump($row);
        return $row;
    }
    ...
    function setStatus($request){
        ...
        $res = D('CarBrand')->where($data)->save($newdata);
        if($res){
