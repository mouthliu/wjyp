 ...
 * 招商人员管理商户 逻辑处理
 */
class MerchantRunLogic extends BaseLogic{
    ...
    function getList($request = array()) {
        ...
        if(!empty($request['is_status']) || $request['is_status'] == '0') {
            $param['where']['admin.is_status']   = $request['is_status'];
        }else{
            $param['where']['admin.is_status']   = array('lt',9);    //状态
        }
        $param['where']['admin.status'] = 1;
        $param['where']['admin.a_id'] = getMerchantId();
        $param['order']             = 'create_time DESC';   //排序
        $param['page_size']         = C('LIST_ROWS');        //页码
        $param['parameter']         = $request;             //拼接参数

        $result = D('MerchantRun')->getList($param);
        //处理经营范围
        return $result;
    }
    ...
    function findRow($request = array()) {
        ...
        $row = D('MerchantRun')->findRow($param);
