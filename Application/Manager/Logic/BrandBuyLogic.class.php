 ...
 * 商品
 */
class BrandBuyLogic extends BaseLogic {
    ...
    function getList($request = array()) {
        ...
        $result = D('BrandBuy')->getList($param);
    ...
    function findRow($request = array()) {
        ...
        //处理图片
        $row['BrandBuy_img'] = api('System/getFiles',array($row['BrandBuy_img']));
        return $row;
    }
...
//        $data['end_time']   = strtotime($data['end_time']);
    ...
    function addBrandBuyGoods($request = array()){
        $param['where']['goods.status'] = 2;//状态
        $param['where']['goods.is_buy'] = 1;//上架的
        $param['where']['goods.is_brand'] = 0;//上架的
        ...
        if(!empty($request['brand_id'])){
            $param['where']['brand_id'] = $request['brand_id'];
        }
