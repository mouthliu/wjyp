...
//
    ...
    function getGoods(){
        ...
        if(!empty($_POST['gid'])){
            $where['id'] = $_POST['gid'];
            $res = $good->field("id,goods_name,merchant_id,shop_price,cat_id")->where($where)->find();

            if($res){
                //创建节点
                $html = "<tr><td class='goods_id'>{$res['id']}</td><td class='goods_name' data-mid='{$res['merchant_id']}'>{$res['goods_name']}</td><td data-price='{$res['shop_price']}' data-catId='{$res['cat_id']}'><a href='javascript:;' class='xuan'>选定</a></td></tr>";
                echo $html;
            }else{
                return false;
            }
        ...
        if(!empty($_POST['gname'])){
            ...
            $res = $good->field("id,goods_name,merchant_id,shop_price,cat_id")->where($where)->select();

            if($res){
                $html = '';
                //创建节点
                foreach($res as $k=>$v){
                    $html .= "<tr><td class='goods_id'>{$v['id']}</td><td class='goods_name' data-mid='{$v['merchant_id']}'>{$v['goods_name']}</td><td data-price='{$v['shop_price']}' data-catId='{$v['cat_id']}'><a href='javascript:;' class='xuan'>选定</a></td></tr>";
                }
