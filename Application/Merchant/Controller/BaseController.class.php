...
//        $data['qrcode'] = $fileName;
    ...
    function afterProcess($array = array()){
        $modArr = array('GroupBuy','LimitBuy','PreBuy','Theme','OneBuy','Auction','IntegralBuy','TicketBuy');
        if (!in_array($array['model'],$modArr)){
            return false;
        }
        if($array['field'] == 'status' && $array['value']==9){
            //删除
    ...
    function setField(){
        ...
        $value = $_POST['value']?$_POST['value']:die('参数不足');
        $data[$field] = $value;
     ...
     * 获取拒绝审核记录
     */
    function getRefuseLog(){
        $where['id_val'] = $_POST['id_val'];
        $where['type'] = $_POST['type'];
        $list = M('RefuseLog')->where($where)->order('create_time DESC')->select();
        $html = '';
        if($list){
            foreach($list as $k=>$v){
                $time = date('Y-m-d H:i',$v['create_time']);
                $html .= "<tr><td>{$time}</td><td>{$v['action_admin']}</td><td>{$v['refuse_desc']}</td></tr>";
            }
            echo $html;
        }else{
            echo $html;
