...
//        dump($attr_img);
    ...
    protected function getAddRelation(){
        ...
        $merchant_template = M('merchant_template')->where($where)->select();
//        p($merchant_template);
        $this->assign('merchant_template',$merchant_template);
        //获取到分类 品牌 进口地
        $mylist = M('Merchant')->field("id,a_id,cates,brands,countrys,merchant_name,range_id")->where("id={$id}")->find();
...
//             }
    ...
    public function audit(){
        if(IS_GET){
//            $info = M('goods')
//                ->join('db_attract_goods ON db_attract_goods.a_id = db_goods.a_id AND db_attract_goods.merchant_id = db_goods.merchant_id AND db_attract_goods.goods_code = db_goods.goods_code AND db_attract_goods.shop_price =db_goods.shop_price AND  db_attract_goods.settlement_price = db_goods.settlement_price AND db_attract_goods.market_price = db_goods.market_price')
//                ->where(array('db_goods.id'=>$_GET['goods_id']))
//            ->find();
//            if(empty($info)){
//               $this->error('商品信息与招商人员预录入商品信息不符,请联系招商人员!');
//           }else{
                $save['status'] = 1;
