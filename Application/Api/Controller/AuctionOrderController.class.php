...
//        $this->returnNotLoginMsg($user_id);
        $user_id=54;
        D('AuctionOrder','Logic')->OrderList(I('post.'),$user_id);
    }
    /**
     * 生成订单（结算页过来）
     * 请求参数:
...
//        1=>付定金  2=>付尾款
        if($_POST['end_type'] == 1) {
            if (empty($_POST['order_id'])) {
                ...
                D('AuctionOrder', 'Logic')->SetOrder(I('post.'), $user_id);
            } else {
                $og                             = M('auction_order')->where(array('id' => $_POST['order_id']))->field('order_price,discount,yellow_discount,blue_discount,merchant_name')->find();
...
//        $this->returnNotLoginMsg($user_id);
        $user_id = 54;
        D('AuctionOrder','Logic')->ShoppingCart(I('post.'),$user_id);
    }
