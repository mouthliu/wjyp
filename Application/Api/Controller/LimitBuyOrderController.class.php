 ...
 *
 */
class LimitBuyOrderController extends BaseController{
    ...
    public function OrderList(){
        ...
        D('LimitBuyOrder','Logic')->OrderList(I('post.'),$user_id);
    }
    /**
     * 生成订单（结算页过来）
     * 请求参数:
...
//        $user_id = 27;
        ...
        if (empty($_POST['limit_buy_order_id'])) {
...
//            }
            D('LimitBuyOrder', 'Logic')->setOrder(I('post.'), $user_id);
        } else {
            $og = M('limit_buy_order')->where(array('id' => $_POST['limit_buy_order_id']))
                ->field('id,order_price,discount,yellow_discount,blue_discount,merchant_name')
                ->find();
            $result_data['limit_buy_order_id']        = $og['id'];
    ...
    public function shoppingCart(){
        $user_id = $this->checkLogin();
        $this->returnNotLoginMsg($user_id);
        D('LimitBuyOrder','Logic')->shoppingCart(I('post.'),$user_id);
    }
    ...
    public function cancelOrder(){
        $user_id = $this->checkLogin();
        $this->returnNotLoginMsg($user_id);
        D('LimitBuyOrder','Logic')->cancelOrder(I('post.'),$user_id);
    }
...
//        $user_id = 27;
        ...
        $og = M('limit_order_goods')->alias('g')->where(array('limit_order_id'=>$_POST['limit_buy_order_id']))
            ->join('db_limit_buy_order as o on g.limit_order_id=o.id')
