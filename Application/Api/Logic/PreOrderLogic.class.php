...
class PreOrderLogic extends BaseLogic{
    /**
     * 订单列表
     * @param int $user_id
     */
     ...
     * order_type  购买渠道
     */
    public function preOrderList($request = array(),$user_id = 0){
...
//            尾款
     ...
     */
    public  function preSetOrder($request = array(),$user_id = 0){
        $mod = M('pre_order');
        $mod->startTrans();//开启事务
            //商家id
...
//            ->join("db_merchant  as m on g.merchant_id = m.id")
            ...
            ->select();
//        判断活动结束时间
        $time = M('pre_buy')->where(array('goods_id'=>$request['goods_id']))->field("start_time,end_time,pre_price,success_min_num,deposit,product_id")->find();
 //            超时
        if($time['end_time']<time()){
            apiResponse('0','预购时间已过');
        }
//        未到
        if(time()<$time['start_time']){
            apiResponse('0','未到预购时间');
        }
//        判断库存
        foreach($goods as $k){
//            if($k['goods_num']<$time['success_min_num']){
//                apiResponse('0','库存不足');
//            }
...
//            if($request['cart_ids'] == 0){
                ...
                $g_data['shop_price'] = $time['pre_price'];
...
//            商品属性整理
                ...
                $g_data['pre_order_id'] = $id;
                $g_data['create_time'] = time();
                $res = M('pre_order_goods')->data($g_data)->add();
                if(!$res){
                    $mod->rollback();
                    apiResponse('0','生成订单失败');
                }else{
                ...
                $order_pic['order_price'] = $g_data['shop_price'];
                $order_pic['pre_order_goods'] = $res;
                $order_pic['order_status'] = 7;
                $order_pic['start_price'] = $time['deposit'];
                $order_pic['final_price'] = $time['pre_price']-$time['deposit'];
                $pic = M('pre_order')->where(array('id'=>$id))->save($order_pic);
                if($pic){
                    $result_data['order_id'] = $id;
                    $result_data['merchant_name'] = $g_data['merchant_name'];
                    $result_data['order_price'] = $order_pic['order_price'];
                    $result_data['start_price']     = $order_pic['start_price'];
...
//            }
     ...
     * Date: 2017-12-5
     */
    public function preShoppingCart($request = array(),$user_id = 0){
