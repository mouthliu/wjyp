...
//        订单状态筛选
        ...
        $order_list = M('integral_order')
            ->where($where)
            ->field("id as order_id,order_status,merchant_name,order_price,merchant_name,time_num")
            ->page($request['p'],',10')
...
//            抽奖期号
            $order_list[$k]['integral_id']=$v['time_num'];
//            合计
            $order_list[$k]['sum_price']='9999999';
            $order_list[$k]['order_goods'] = M('integral_order_goods')
                ->where(array('order_id'=>$v['order_id']))
                ->field("order_id,goods_name,merchant_name,shop_price,goods_num,goods_attr,goods_img")
                ->select();
...
//                定金
    ...
    public  function SetOrder($request = array(),$user_id = 0){
        $mod = M('integral_order');
        $mod->startTrans();//开启事务
//        $return_list=array();
//        for($i=0;$i<$request['goods_num'];$i++) {
            //商家id
            $goods_where['id'] = $request['goods_id'];
            $goods             = M('goods')->where($goods_where)
                ->field("merchant_id,merchant_name ,goods_name,id,shop_price,goods_num,goods_img,id")
                ->select();
//        获取活动商品信息
            $time = M('one_buy')->where(array('goods_id' => $request['goods_id']))
                ->find();
            $data['time_num'] = $time['time_num'];
...
//                if ($request['cart_ids'] == 0) {
                    ...
                    $g_data['start_price']   = $time['deposit'];
                    $g_data['final_price']   = $time['pre_price'] - $time['deposit'];
...
//                商品属性
                        $attr_goods               = M('products')->where(array('id' => $request['product_id']))->field('goods_attr,product_number')->find();
...
//            商品属性整理
                    ...
                    $g_data['one_buy_id']  = $time['id'];
                    $g_data['order_id']    = $id;
                    $g_data['create_time'] = time();
                    $g_data['number'] = substr(time(),'0','6').rand(100,999);
                    $res                   = M('integral_order_goods')->data($g_data)->add();
                    if (!$res) {
                        $mod->rollback();
                        apiResponse('0', '生成订单失败');
                    } else {
                    ...
                    //代金券
                    $order_pic                         = array();
                    $order_pic['discount']             = $time['red_discount_num'] * $request['goods_num'];
                    $order_pic['yellow_discount']      = $time['yellow_discount_num']* $request['goods_num'];
                    $order_pic['blue_discount']        = $time['blue_discount_num']* $request['goods_num'];
                    $order_pic['balance_num']          = $time['balance_num']* $request['goods_num'];
                    $order_pic['integral_num']         = $time['integral_num']* $request['goods_num'];
                    $order_pic['integral_order_goods'] = $res;
                    $order_pic['goods_num'] = $request['goods_num'];
                    $order_pic['order_status']         = 0;
                    $pic                               = M('integral_order')->where(array('id' => $id))->save($order_pic);
                    if ($pic) {
//                        加入关系表
                        $inte_num['inte_order_id'] = $id;
                        $inte_num['inte_order_goods_id'] = $pic;
                        $inte_num['goods_num'] = $request['goods_num'];
                        $inte_num['create_time'] = time();
                        $inte = M('integral_num')->data($inte_num)->add();
                        if($inte){
                        $result_data['order_id']      = $id;
                        $result_data['merchant_name'] = $g_data['merchant_name'];
                        $result_data['merchant_id']   = $g_data['merchant_id'];
                        $result_data['red_desc']      = "本产品需使用" . $order_pic['discount'] . "红券";
                        $result_data['yellow_desc']   = "本产品需使用" . $order_pic['yellow_discount'] . "黄券";
                        $result_data['balance_num']   = "本产品需使用" . $order_pic['balance_num'] . "余额";
                        $result_data['integral_num']  = "本产品需使用" . $order_pic['integral_num'] . "积分";

                        //                    余额
                        $balan                  = M('user')->where(array('id' => $user_id))->field('balance')->select();
                        $result_data['balance'] = $balan[0]['balance'];
//                        $return_list[$i]=$result_data;
...
//        }
    ...
    function balancePay($request = array(),$user_id){
        //根据订单id获取到支付的类型,价格
        $type=array();
        $order_info = M('integral_order')->where("id={$request['order_id']} AND user_id={$user_id}")->find();
        ...
        $mod->startTrans();
        //获取当前会员基本信息
        $balance = $mod->where("id={$user_id}")->field('balance,integral')->find();
//        余额
        if($order_info['balance_num']==0){
           $type['balance_num'] = 0;
        }
//        积分
        if($order_info['integral_num']==0){
            $type['integral_num'] = 0;
        }
//        红券
...
//        黄券
        if($order_info['yellow_discount']==0){
            $type['yellow_discount'] = 0;
        }
//        蓝券
        if($order_info['blue_discount']==0){
            $type['blue_discount'] = 0;
        }
//        余额  1
        if($request['pay_type'] == 1){
            $pay = $order_info['balance_num'] * $order_info['goods_num'];
            if($balance['balance'] < $pay){
                apiResponse('0','余额不足');
            }else{
                $money = $pay;
                $act_type = 3;
        ...
        if($request['pay_type'] == 2){
            $pay = $order_info['integral_num'] * $order_info['goods_num'];
            if($balance['integral'] < $pay){
                apiResponse('0','积分不足');
            }else{
                $money = $pay;
                $act_type = 10;
...
//            （1=> 汽车,2=>房产,3=>'直接购买',4=>'预购购买',5=>'团购',6=>'拍卖'）
    ...
    public function CancelOrder($request = array(),$user_id=0){
        ...
        $order_info = M('integral_order')->where(array('id'=>$request['order_id']))->find();
        if(!$order_info){
            apiResponse('0','订单信息不存在');
        }
        $save['update_time'] = time();
        $save['order_status'] = 5;
        $data = M('integral_order')->where(array('id'=>$request['order_id']))->save($save);
        if($data){
    ...
    public function DeleteOrder($request=array(),$user_id){
        ...
        $save['order_status'] = 9;
        $save['update_time'] = time();
        $data = M('integral_order')->where("id = {$request['order_id']}")->save($save);
        if($data){
            apiResponse('1','删除成功');
        }else{
            apiResponse('0','删除失败');
