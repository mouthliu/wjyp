...
//        订单状态筛选
        ...
        foreach($order_list as $k =>$v){
            $order_list[$k]['order_goods'] = M('order_goods')
...
//            直接购买
            //商家id
            $goods_where['id'] = array('in',$request['cart_ids']);
        }else{
//            购物车
...
//            商家名称
        ...
        if($id){
            //订单号
            $s_data['order_sn'] = time().rand(10000,99999);
...
//库存
                ...
                $g_data['order_id'] = $id;
                $og_creat = D('OrderGoods')->create($g_data);
                if($og_creat){
                    $res = D('OrderGoods')->data($og_creat)->add();
                }
                if(!$res){
                    $mod->rollback();
                    apiResponse('0','生成订单失败');
                ...
                $pic = M('order')->where(array('id'=>$id))->save($order_pic);
                if($pic){
//                    $cart_list['sum_discount'] = $order_pic['discount']+$order_pic['yellow_discount']+$order_pic['blue_discount'];
...
//                $result_data['is_integral'] = $order_pic['is_integral'];
//                $result_data['integral'] = $user_info['integral'];
                ...
                //说明是从购物车过来,根据购物车里面的商品信息进行存储
//                获取商品信息
                $where_['c.id'] = array('in',$request['cart_ids']);
                $cart = M('cart')->alias('c')->where($where_)
                    ->join("db_goods  as g ON c.goods_id = g.id")
//                    ->field("")
                    ->select();
//                订单总额
                $all_pic = 0;
//                判断是否有product_id
                foreach($cart as $c_k => $c_v){
                    if($c_v['product_id'] != 0){
//                        判断是否存在商品属性
//                        存在
//                        获取属性值，进行拼接
                        $attr = M('products')->where(array('id'=>$c_v['product_id']))->select();
                        $attr_goods['goods_attr'] = explode('|',$attr[0]['goods_attr']);
//            库存
                        $attr_goods['product_number'] = $attr[0]['product_number'];
...
//                        拼接好属性
                        $g_data['goods_attr']=rtrim($g_data['goods_attr'],'|');
//                        价格进行处理
                        $g_data['shop_price'] = $c_v['shop_price'] +$else_price;
                        $g_data['product_id'] = $attr[0]['id'];

                    }else{
//                        不存在
                        $g_data['goods_attr'] = '';
//                        库存
                        $attr_goods['product_number'] = $c_v['goods_num'];
//                        价格
                        $g_data['shop_price'] = $c_v['shop_price'];
                        $g_data['product_id']=0;
                    }
                    //                        总额
                    $all_pic+=$g_data['shop_price']*$c_v['num'];
//                    拼字段
                    $g_data['goods_sn'] = $c_v['goods_sn'];
                    $g_data['order_id'] = $id;
                    $g_data['goods_name'] = $c_v['goods_name'];
                    $g_data['goods_id'] = $c_v['goods_id'];
                    $g_data['goods_num'] = $c_v['num'];
                    $g_data['merchant_name'] = $c_v['merchant_name'];
                    $g_data['merchant_id'] = $c_v['merchant_id'];
                    $g_data['market_price'] = $c_v['market_price'];
                    $g_data['goods_img'] = $c_v['goods_img'];
                    $g_data['total'] = $all_pic;
                    $og_creat = D('OrderGoods')->create($g_data);
                    if($og_creat){
                        $res = D('OrderGoods')->data($og_creat)->add();
                    }
...
//                    代金券
                    $order_pic=array();
                    $order_pic['discount'] += sprintf("%.2f", $c_v['discount']/100*$c_v['shop_price']*$g_data['goods_num']);
                    $order_pic['yellow_discount'] += sprintf("%.2f", $c_v['yellow_discount']/100*$c_v['shop_price']*$g_data['goods_num']);
                    $order_pic['blue_discount'] += sprintf("%.2f", $c_v['blue_discount']/100*$c_v['shop_price']*$g_data['goods_num']);

                }
                $order_pic['order_price'] = $all_pic;
                $order_pic['sum_discount'] = $order_pic['discount']+$order_pic['yellow_discount']+$order_pic['blue_discount'];
                $pic = M('order')->where(array('id'=>$id))->save($order_pic);
                if($pic){
//                    $cart_list['sum_discount'] = $order_pic['discount']+$order_pic['yellow_discount']+$order_pic['blue_discount'];
...
//            }
            if(empty($request['merchant_id'])){
                apiResponse('0','请传商家id');
            }
            if(empty($request['cart_id'])){
                apiResponse('0','请传购物车id');
            }else{
                $where['db_cart.id'] = array('in',$request['cart_id']);
            }
            //购物车商品列表
            $cart_list['item'] = M('cart')
                ->join('db_goods ON db_cart.goods_id = db_goods.id')
                ->where($where)
                ->order('db_cart.create_time desc')
//                ->page($request['p'].',10')
                ->field('db_cart.id as cart_id,db_cart.goods_id,goods_name,goods_img,num,shop_price,discount,yellow_discount,blue_discount,product_id')
                ->select();
        }else{
...
//        p($cart_list);die;
    ...
    public function deleteOrder($request=array(),$user_id){
        ...
        $data = M('order')->where("id = {$request['order_id']}")->save($save);
        if($data){
            apiResponse('1','删除成功');
        }else{
            apiResponse('0','删除失败');
        ...
        switch($request['order_type']){
            ...
            case 2:
                $goods_list = M('group_order_goods')
                    ->where(array('group_order_id'=>$request['order_id']))
                    ->field('id as order_goods_id,goods_id,goods_name,product_id')
                    ->select();
                break;
            case 3:
