        ...
        if(is_array($info)) {
            foreach ($info as $key => &$value) {
                ...
                if($id = M('File')->add($value)) {
                    ...
                    $value['download_url'] = U('UpDownLoad/download',array('id'=>$value['id']));
                    //本地上传成功后再上传到OSS服务器
//                    $rs = ossUpload($value['path']);
                    //如果上传不成功删除本地上传的记录
//                    if($rs){
//                        M('File')->where("id = {$id}")->save(array("oss_path"=>$rs));
//                    }
                } else {
     ...
     *$isThumb:是否缩略图
     */
    function ossUpPic($fFiles,$n,$ossClient,$bucketName,$web)
    {
        $fType = $fFiles['type'];
        $back  = array(
            'code' => 0,
        ...
        $ext   = substr($fFiles, stripos($fFiles, '.'));
        $fup_n  = $fFiles;
    ...
    function upPic($file){
        ...
        //图片
        $fFiles=$file;
        $rs=ossUpPic($fFiles,'s',$ossClient,$bucketName,$web);
        if($rs['code']==1){
            //图片
            return $rs['msg'];
        }else{
