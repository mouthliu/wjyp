...
class SystemApi {
    ...
    public static function sendMsg($receiver, $unique_code, $params, $sender = 0, $content = '', $type = 0, $subject = '') {
        ...
        if(!empty($unique_code)) {
            ...
            $tpl = M('SendTemplate')->field('id,type,subject,template,status,template_code')->where(array('unique_code'=>$unique_code,'status'=>array('lt',9)))->find();
            if($tpl) {
                if($tpl['status'] == 0) {
        ...
        //获取接收会员ID
        $m_id = M('User')->where(array('phone'=>$receiver))->getField('id');
        ...
        if($type == 1) {
            ...
            //发送阿里云短信
            $res = api('Sms/sendSms',array('无界优品',$tpl['template_code'],$receiver,Array("vc"=>$params['vc'], "product"=>"无界优品")));
        } elseif($type == 2) {
            //验证邮箱格式
